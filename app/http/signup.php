...
if ($stmt->rowCount() > 0) {
	...
	if (isset($new_img_name)) {
		...
		$sql = "INSERT INTO users (name, username, password,question,answer, p_p) VALUES (?,?,?,?,?,?)";
		$stmt = $conn->prepare($sql);
		$stmt->execute([$name, $username, $password, $question, $answer, $new_img_name]);
	} else {
		# inserting data into database
		$sql = "INSERT INTO users (name, username, password,question,answer) VALUES (?,?,?,?,?)";
		$stmt = $conn->prepare($sql);
		$stmt->execute([$name, $username, $password, $question, $answer]);
	}
